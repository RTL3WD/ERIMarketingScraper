...
def download_pdfs(link,records):
    ...
    optionsUC.add_argument('--window-size=920,1280')
    ...
    if not merchant_type:
        sleep(3)
        elemntDriver.quit()
        print('Document doesn\'t meet criteria. Returning')
        return
    print('Merchant type doc found')
    
    # with open('test_docs.json','w') as f:
    #     f.write(json.dumps(test_docs, indent=2))
    ...
    if folder != '':
        ...
        if 'price' in record and record['price'] >= 20000: 
            ...
            #     })
                ...
                #     leads_data = json.loads(f.read())
                # leads_data.append(lead)
                # with open(leads_file, 'w') as f:
                #     f.write(json.dumps(leads_data, indent=2))
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                driver.quit()
                print(f'Submitting {len(links)} to threadpool')
                logger.info(f'Submitting {len(links)} to threadpool')
                driver.quit()
                for link in links:
                    download_pdfs(link, records)

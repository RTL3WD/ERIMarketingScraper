        ...
        try:
            ...
            try:
                ...
                for i,e in enumerate([elements[0],elements[1],elements[2]]):
                    ...
                    elemntDriver = Chrome()
                    if(i==0):
                        href = 'https://iapps.courts.state.ny.us/nyscef/DocumentList?docketId=zQD2/s9RZWPxYIwzQ0DUjw==&display=all&courtType=Kings%20County%20Supreme%20Court&resultsPageNum=1'
                    elemntDriver.get(href)
                    docs = elemntDriver.find_elements(By.CSS_SELECTOR, 'table.NewSearchResults > tbody > tr > td:nth-child(2) > a')
                    contentName= elemntDriver.find_element(By.CSS_SELECTOR, '#content').get_attribute('innerHTML').strip()
                    hrefs = []
